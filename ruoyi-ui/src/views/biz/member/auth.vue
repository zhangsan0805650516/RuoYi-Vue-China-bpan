<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item :label="$t('审核状态')" prop="isAuth">
        <el-select
      ...
      <el-form-item :label="$t('创建时间')" prop="submitAuthTime">
        <el-date-picker
          clearable
          v-model="dateRange"
          type="daterange"
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          :disabled="multiple"
          @click="batchAuthMember"
          v-hasPermi="['biz:member:authMemberBatch']"
        >{{$t('批量实名认证')}}</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="memberList" @selection-change="handleSelectionChange" @sort-change="handleSortChange">
      ...
      <el-table-column :label="$t('实名姓名')" align="center" prop="name" />
      <el-table-column :label="$t('手机号')" align="center" prop="mobile" >
        <template slot-scope="scope">
          {{ scope.row.mobile ? scope.row.mobile.replace(/(\d{3})(\d{4})(\d{4})/, "$1****$3") : "" }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('身份证号码')" align="center" prop="idCard" width="200" />
      <el-table-column :label="$t('正面身份证')" align="center" prop="idCardFrontImage" >
        <template slot-scope="scope">
          <image-preview :src="scope.row.idCardFrontImage" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('反面身份证')" align="center" prop="idCardBackImage" >
        <template slot-scope="scope">
          <image-preview :src="scope.row.idCardBackImage" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('驳回原因')" align="center" prop="authRejectReason" />
      <el-table-column :label="$t('创建时间')" align="center" prop="submitAuthTime" width="200">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.submitAuthTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('审核状态')" align="center" prop="isAuth" >
        <template slot-scope="scope">
          <el-tag type="" v-if="scope.row.isAuth === 1">审核中</el-tag>
          <el-tag type="success" v-if="scope.row.isAuth === 2">审核通过</el-tag>
          <el-tag type="danger" v-if="scope.row.isAuth === 3">驳回</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('操作')" align="center" class-name="small-padding fixed-width" fixed="right" width="120">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="authMember(scope.row)"
            v-hasPermi="['biz:member:authMember']"
    ...
    <!-- 新增身份证认证 -->
    <el-dialog :title="title" :visible.sync="openAddAuth" width="600px" append-to-body>
...
export default {
  ...
  data() {
    return {
      ...
      // 查询参数
      queryParams: {
        orderByColumn: 'submit_auth_time',
        isAsc: 'descending',
        pageNum: 1,
        pageSize: 10,
        id: null,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      authMemberList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleAdd() {
      this.reset();
      this.openAddAuth = true;
      this.title = this.$t("实名认证");
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
